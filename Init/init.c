...
int main(void) {
    ...
    // mount target pivot directories
    //
    ...
    if(fgets(make, sizeof(make), make_file) != NULL){
    	make[strcspn(make, "\n")] = '\0';
    }
    ...
    int sethostname_return = sethostname(hostname,sizeof(hostname));
    ...
    if(sysinfo(&info) == 0){
    	...
    	fclose(f);
    	pid_t mk_zram_swap_pid=fork();
    	if(mk_zram_swap_pid==0){
    		char *mk_zram_swap_array[] = {"mkswap", "/dev/zram0",  NULL};
    		printf("mkswap /dev/zram0\n");
		execve("/usr/bin/mkswap", mk_zram_swap_array, root_environment);
		exit(0);
    	}else{
		waitpid(mk_zram_swap_pid,NULL,0);
    	}	
	pid_t zram_swap_on_pid=fork();
    	if(zram_swap_on_pid==0){
    		char *zram_swap_on_array[] = {"swapon", "/dev/zram0",  NULL};
    		printf("swapon /dev/zram0\n");
		execve("/usr/bin/swapon", zram_swap_on_array, root_environment);
		exit(0);
    	}else{
		waitpid(zram_swap_on_pid,NULL,0);
    	}
    }
    ...
    system("/usr/bin/setfacl -m u:user:rw /dev/rfkill");
